...
mt7615_ampdu_stat_read(struct seq_file *file, void *data)
{
	...
	int bound[8], i, range0, range1;

	range0 = mt76_rr(dev, MT_AGG_ASRCR0);
	range1 = mt76_rr(dev, MT_AGG_ASRCR1);
	for (i = 0; i < 4; i++) {
		bound[i] = MT_AGG_ASRCR_RANGE(range0, i) + 1;
		bound[i + 4] = MT_AGG_ASRCR_RANGE(range1, i) + 1;
	}
